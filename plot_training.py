...
def plot_all_metrics(models):
    ...
    for idx, model_name in enumerate(models):
        ...
        ax = axes[0, 0]
        ax.plot(epochs, history['train_loss'], label=f'{label} - Train', color=color, linewidth=2)
        ax.plot(epochs, history['val_loss'], label=f'{label} - Val', color=color, linestyle='--', linewidth=2)
        ax.set_xlabel('Epoch')
        ax.set_ylabel('Loss')
        ...
        if 'learning_rate' in history:
            ax.plot(epochs, history['learning_rate'], label=label, color=color, linewidth=2, marker='o', markersize=4)
            ax.set_xlabel('Epoch')
        ...
        if 'grad_norm' in history:
            ax.plot(epochs, history['grad_norm'], label=label, color=color, linewidth=2)
            ax.set_xlabel('Epoch')
        ...
        ax.plot(epochs, improvement, label=label, color=color, linewidth=2, marker='o', markersize=4)
        ax.set_xlabel('Epoch')
        ...
        ax.plot(epochs, gap, label=label, color=color, linewidth=2)
        ax.set_xlabel('Epoch')
    ...
    for model_name in models:
        ...
        final_lr = history['learning_rate'][-1] if 'learning_rate' in history else 'N/A'
        total_epochs = len(history['train_loss'])
        summary_data.append(
            [model_name.upper(), f"{best_val_loss:.4f}", f"{best_epoch}", f"{total_epochs}", f"{final_lr}"])
    ...
    print("✓ Saved training_dynamics_comparison.png")
    plt.show()
    ...
    for model_name in models:
        try:
            ...
            comparison_data.append({
                'Model': model_name.upper().split("_")[1],
...
if __name__ == '__main__':
        ...
        models_to_plot = ['stanford_resnet18', 'stanford_resnet50', 'stanford_efficientnet_b0']

        # Check which models have history files
        available_models = [model for model in models_to_plot if Path(f'results/{model}_history.json').exists()]

        if not available_models:
            print("No training history found for the specified models! Train models first.")
            exit(1)
